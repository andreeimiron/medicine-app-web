...
const Consultation = ({ fields, onCreate, patient, onUserError,  }) => {
  ...
  const schema = {
    ...
    observations: Joi.string().allow(null, '').label(data.observations.label),
  ...
  const onSubmit = async (event) => {
    ...
    const certificate = {
      ...
      observations: data.observations.value,
    ...
    if (formIsValid) {
      ...
      try {
        ...
        onCreate();
        setData(fields);
        showSuccess('Consultatia a fost inregistrata cu succes');
      } catch (error) {
        showError(error);
      }
