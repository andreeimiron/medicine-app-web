...
const Certificate = ({ fields, onCreate }) => {
  ...
  const schema = {
    ...
    recomandation: Joi.string().allow(null, '').label(data.recomandation.label),
    releaseReason: Joi.string().required().label(data.releaseReason.label),
    ...
    ableFor: Joi.string().allow(null, '').label(data.ableFor.label),
    result: Joi.string().allow(null, '').label(data.result.label),
    conclusion: Joi.string().allow(null, '').label(data.conclusion.label)
  };
  ...
  const onSubmit = async (event) => {
    ...
    if (formIsValid) {
      ...
      try {
        ...
        showSuccess('Adeverinta a fost trimisa cu succes catre pacient');
      } catch (error) {
        showError(error);
      }
