...
const Vaccine = ({ fields, onCreate }) => {
  ...
  const schema = {
    ...
    description: Joi.string().required().label(data.description.label),
  ...
  const onSubmit = async (event) => {
    ...
    const result = Joi.validate(vaccine, schema, { abortEarly: false });
    ...
    if (formIsValid) {
      ...
      try {
        vaccine.userId = selectedUser.id;
        await create(vaccine);

        onCreate();
        setData(fields);
        setSelectedUser(null);
        showSuccess('Vaccinul a fost inregistrat.');
      } catch (error) {
        showError(error);
      }
